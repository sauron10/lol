...
const profileIconPopulator = async () => {
  try{
    const file = '../general/12.12.1/data/en_US/profileicon.json'
...
const championPopulator = async () => {
  try{
    const file = '../general/12.12.1/data/en_US/championFull.json'
...
const itemPopulator = async () => {
  const file = '../general/12.12.1/data/en_US/item.json'
    const f = fs.readFile(file, async(err, data) => {
      if(err) throw err
...
const summonerSpellPopulator = async() => {
  const file = '../general/12.12.1/data/en_US/summoner.json'
    const f = fs.readFile(file, async(err, data) => {
      if(err) throw err
...
const runesPopulator = async() => {
  const file = '../general/12.12.1/data/en_US/runesReforged.json'
    const f = fs.readFile(file, (err, data) => {
      if(err) throw err
