...
export const useGetSummoner = summonerName => {
  ...
  const updateData = useCallback(async(num,queue,champion='') => {
    try{
      ...
      const res = await axios.get(`https://www.lstats.xyz/api/${summonerName}/?start=0&queue=${queue}&size=${num}&champion=${champion}`)
  ...
  const getSeasonMatches = async() =>{
    try{
      setLoaded(false)
      const res = await axios.get(`https://www.lstats.xyz/api/${summonerName}/season/?startTime=1641549600`)
      return res
    }catch(e){
  ...
  const updateProfileData = async (selectedTab) => {
    try{
      setLoaded(false)
      const res = await axios.get(`https://www.lstats.xyz/api/${summonerName}/update/?queue=${selectedTab}`)
  ...
  useEffect(() => {
    const getData = async (summName) => {
      try {
        return await axios.get(`https://www.lstats.xyz/api/${summName}/`);
      } catch (e) {
